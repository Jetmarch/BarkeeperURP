...
public class SpectralFluxAnalyzer {
	...
	public void SetMusicBeatEvent(SOEvent musicBeat)
	{
		this.musicBeat = musicBeat;
	}

	public void SetThresholdWindowSize(int value)
    {
		this.thresholdWindowSize = value;
    }

	public void SetThresholdMultiplier(float value)
    {
		this.thresholdMultiplier = value;
    }
	...
	public void analyzeSpectrum(float[] spectrum, float time) {
		...
		if (spectralFluxSamples.Count >= thresholdWindowSize) {
			...
			if (curPeak) {
				...
				//Invoke event of music beat here
				musicBeat.Raise();
			}
			indexToProcess++;
		}
