...
public class AudioProcessor : MonoBehaviour
{
	...
	private void initArrays ()
	{
		...
		averages = new float[averagesCount];
	...
	public void computeAverages (float[] data)
	{
		for (int i = 0; i < averagesCount; i++) {
			...
			if (i == 0)
				...
				lowFreq = (int)((samplingRate / 2) / (float)System.Math.Pow (2, averagesCount - i));
			int hiFreq = (int)((samplingRate / 2) / (float)System.Math.Pow (2, (averagesCount - 1) - i));
